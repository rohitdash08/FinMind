...
def render_frame(lines):
    ...
    draw.text((60, 5), "FinMind - Multi-account Overview Demo", fill=FG_COLOR, font=font)
    y = 24 + PADDING
...
def main():
    with open("demo_132.cast", "r", encoding="utf-8") as f:
    ...
    images[0].save("demo_accounts.gif", save_all=True, append_images=images[1:], duration=durations, loop=0, optimize=True)
    print(f"GIF saved: demo_accounts.gif ({os.path.getsize('demo_accounts.gif')/1024:.0f} KB, {len(images)} frames)")
