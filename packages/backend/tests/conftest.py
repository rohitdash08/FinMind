...
def app_fixture():
    ...
    patches = [
        unittest.mock.patch("app.extensions.redis_client", fake_redis),
        unittest.mock.patch("app.routes.auth.redis_client", fake_redis),
        unittest.mock.patch("app.services.login_anomaly.redis_client", fake_redis),
    ]
    for p in patches:
        p.start()
