...
def test_weekly_digest_rejects_invalid_week(client, app_fixture):
    auth = _register_and_auth_header(client, app_fixture, "digest-invalid@example.com")
    r = client.get("/insights/weekly-digest?year=2025&week=60", headers=auth)
    assert r.status_code == 400
    assert r.get_json()["error"] == "invalid ISO week or year"
